...
const getMisPublicaciones = (req, res) => {
    ...
    connection.query(sql, [usuario_id, usuario_id, usuario_id], (err, results) => {
        ...
        res.render('publicaciones', { username: req.session.username, userid: usuario_id, publicaciones: results, tipo: 'mis_publicaciones', resultado: 1});
    });
};
        ...
        results.forEach((publicacion) => {
            ...
            connection.query(sqlComentarios, [publicacion.publicacion_id], (err, comentarios) => {
                ...
                if (publicacionesConComentarios.length === results.length) {
                    res.render('publicaciones', { tipo: 'publicaciones', username: req.session.username, userid, publicaciones: publicacionesConComentarios,resultado: 1 });
                }
            });
        });
    };
